...
1. 使用gitlab内部的CI/CD部署方案
1. 使用其他轻量化的方案(类似执行一条自定义命令 `npm run deploy` 就可以实现打包操作)
...
1. **初始化**
初始化需要在 `github` 上新建一个部署配置`git仓库`，执行`deploy init`通过`download-git-repo`从`git`上拉取配置模板。
