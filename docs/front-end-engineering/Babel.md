...
虽然使用transform-runtime后文件大小会有所增大，但是解决一些兼容性的问题。
...
> 在 `Babel > 7.4.0` 之前，通常我们会安装 `babel-polyfill` 或 `@babel/polyfill` 来处理实例方法和 ES+新增的内置函数，而`7.4.0之`后，当我们选择安装 `@babel/polyfill` 时，会收到如下的警告 ：
```text
